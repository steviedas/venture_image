...
VIDEO_EXTS = {".mp4", ".mov", ".m4v", ".mkv", ".avi", ".wmv", ".3gp", ".webm", ".mts", ".m2ts", ".ts"}
...
def ffprobe_stream_info(ffprobe_bin: str, src: Path) -> tuple[dict | None, dict | None, int | None]:
    """
    ...
    try:
        out = subprocess.check_output(
            [
                ffprobe_bin, "-v", "error",
                "-show_entries", "stream=index,codec_name,codec_type,pix_fmt,profile,width,height",
...
def should_route_cpu(codec: str | None, pix_fmt: str | None) -> bool:
    ...
    cpu_codecs = {
        "prores", "prores_aw", "prores_ks", "dnxhd", "dnxhr",
        "jpeg2000", "j2k", "qtrle", "png", "ffv1", "huffyuv",
    }
...
def nvenc_args(preset: str, crf: int, gpu_index: int | None) -> list[str]:
    ...
    nvenc_preset = {
        ...
        "slow": "p7", "slower": "p7", "veryslow": "p7", "placebo": "p7",
    }.get(preset, "p6")
    ...
    try:
        if proc.stderr:
            ...
            for line in err.splitlines():
                low = line.lower()
                if "nvenc" in low or "error" in low or "failed" in low or "capable devices" in low:
                    reason = line.strip()
                    break
    except Exception:
        pass
...
def encode_one(job: Job, backend: str, scen: Scenario, overwrite: bool) -> tuple[bool, str | None]:
    dst = job.dst
    ...
    v_args = nvenc_args(scen.preset, scen.crf, scen.gpu_index) if backend == "nvenc" else x264_args(scen.preset, scen.crf, scen.x264_threads_per_job)
    a_args = choose_audio_args(scen.audio_mode, job.acodec, "320k")

    cmd = ["ffmpeg", "-hide_banner", "-nostdin", *flags, *pre_input,
           "-i", str(job.src), "-map_metadata", "0",
           *v_args, *a_args, "-movflags", "+faststart", "-v", "error", str(dst)]
...
def encode_one_gpu_with_fallback(job: Job, scen: Scenario) -> tuple[bool, str | None, bool]:
    ...
    try:
        ...
        pass
    scen_cpu = Scenario(
        name=scen.name, strategy=scen.strategy, preset=scen.preset, crf=scen.crf,
        nvenc_workers=None, cpu_workers=scen.cpu_workers,
        backend="cpu", gpu_index=None,
        audio_mode=scen.audio_mode, cuda_decode=False,
...
def staged_run(jobs_gpu: list[Job], jobs_cpu: list[Job], scen: Scenario) -> Result:
    ...
    # Stage 1: GPU queue
    if scen.backend == "nvenc" and jobs_gpu and scen.nvenc_workers and scen.nvenc_workers > 0:
        with cf.ThreadPoolExecutor(max_workers=scen.nvenc_workers) as ex:
            futs = {ex.submit(encode_one_gpu_with_fallback, j, scen): j for j in jobs_gpu}
    ...
    return Result(scenario=scen, elapsed=elapsed, total=len(jobs_gpu) + len(jobs_cpu),
                  converted=converted, skipped=skipped, gpu_fallbacks=gpu_fallbacks, failures=failures)
...
def dual_run(jobs_gpu: list[Job], jobs_cpu: list[Job], scen: Scenario) -> Result:
    ...
    ex_gpu = cf.ThreadPoolExecutor(max_workers=scen.nvenc_workers) if (scen.backend == "nvenc" and scen.nvenc_workers and jobs_gpu) else None
    ex_cpu = cf.ThreadPoolExecutor(max_workers=max(1, os.cpu_count() or 4)) if jobs_cpu else None
...
def main() -> None:
    if which("ffmpeg") is None:
        print("ffmpeg_not_found — install FFmpeg and ensure it’s on PATH.", file=sys.stderr)
        sys.exit(1)
    ffprobe_bin = which("ffprobe") or "ffprobe"

    files = [p for p in gather_files(SOURCE_DIR)]
    if not files:
            ...
            for audio_mode in AUDIO_MODES:
                for cuda_dec in CUDA_DECODE_VARIANTS:
                    scenarios.append(Scenario(
                        ...
                        strategy="staged", preset=PRESET, crf=CRF,
                        nvenc_workers=min(nvw, 8), cpu_workers=max(1, os.cpu_count() or 4),
                        backend="nvenc", gpu_index=gpu_index,
                        audio_mode=audio_mode, cuda_decode=cuda_dec,
                    ...
                    scenarios.append(Scenario(
                        ...
                        strategy="dual", preset=PRESET, crf=CRF,
                        nvenc_workers=min(nvw, 8), cpu_workers=max(1, os.cpu_count() or 4),
                        backend="nvenc", gpu_index=gpu_index,
                        audio_mode=audio_mode, cuda_decode=cuda_dec,
        ...
        for th in CPU_X264_THREADS_PER_JOB_VARIANTS:
            for audio_mode in AUDIO_MODES:
                scenarios.append(Scenario(
                    ...
                    strategy="cpu_only", preset=PRESET, crf=CRF,
                    nvenc_workers=None, cpu_workers=max(1, os.cpu_count() or 4),
                    backend="cpu", gpu_index=None,
                    audio_mode=audio_mode, cuda_decode=False,
    ...
    for scen in scenarios:
        ...
        scen_jobs: list[Job] = []
        for j in base_jobs:
            new_name = j.src.name if j.src.suffix.lower() == ".mp4" else j.src.with_suffix(".mp4").name
            scen_jobs.append(Job(
                src=j.src,
        ...
        print(f"\n==> Running {scen.name}")
        print(f"   files: total={len(scen_jobs)}  gpu_queue={len(gpu_jobs)}  cpu_queue={len(cpu_jobs)}")
        ...
        print(f"   elapsed={res.elapsed:.2f}s  converted={res.converted}  skipped={res.skipped}  gpu_fallbacks={res.gpu_fallbacks}  rate≈{rate:.2f} files/s")
    ...
    for r in results:
        ...
        print(f"{r.scenario.name:40s}  elapsed={r.elapsed:8.2f}s  ok={r.converted:5d}  skip={r.skipped:5d}  fallbacks={r.gpu_fallbacks:4d}  rate={rate:6.2f}/s")
        if (best is None) or (r.elapsed < best.elapsed and r.converted == r.total):
            best = r
    if best:
        print("\nFASTEST (among scenarios that converted all files):")
        print(f"  {best.scenario.name}  elapsed={best.elapsed:.2f}s  files={best.converted}")
    else:
