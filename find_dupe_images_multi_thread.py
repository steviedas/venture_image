#!/usr/bin/env python3
"""
Multithreaded near-duplicate image cleaner (per leaf directory).

For each leaf directory (a folder with no subfolders) under PATH_TO_PROCESS:
  - Compute a perceptual hash (dHash) for all images IN PARALLEL
  - Group images whose hashes are within HAMMING_THRESHOLD
  - Keep the highest-resolution image IN PLACE, move others to ./duplicate/

Parallelism:
  - Image hashing within a leaf uses a thread pool (IO-bound).
  - Leaf directories themselves can be processed in parallel if desired.
...
# Threading
MAX_WORKERS_PER_LEAF  = max(4, (os.cpu_count() or 4) * 4)  # for hashing images (IO-bound)
MAX_WORKERS_FOR_LEAFS = min(8, max(1, (os.cpu_count() or 4)))  # process multiple leaf dirs in parallel
# ====================================================
...
def iter_leaf_dirs(root: Path) -> Iterable[Path]:
    ...
    We don't descend into EXCLUDE_DIR_NAMES (e.g., 'duplicate').
    """
...
def process_leaf_dir(leaf: Path) -> None:
    ...
    # --- Parallel load + hash images ---
    infos: list[ImgInfo] = []
    with cf.ThreadPoolExecutor(max_workers=MAX_WORKERS_PER_LEAF) as pool:
        futures = [pool.submit(load_image_info, f, HASH_SIZE) for f in files]
    ...
    for group in clusters:
        ...
        # Move duplicates sequentially (to keep destination naming deterministic)
        for d in dupes:
...
def main() -> None:
    ...
    log_info(
        f"DRY_RUN: {DRY_RUN} | HASH_SIZE: {HASH_SIZE} | THRESHOLD: {HAMMING_THRESHOLD} | "
        f"PER-LEAF WORKERS: {MAX_WORKERS_PER_LEAF} | LEAF WORKERS: {MAX_WORKERS_FOR_LEAFS}"
    )

    leafs = [p for p in iter_leaf_dirs(root) if p.name not in EXCLUDE_DIR_NAMES]
    if not leafs:
        log_warn("No leaf directories found (directories without subfolders).")
        return

    # --- Option A: process leaves in parallel ---
    # Comment the parallel block and uncomment the sequential loop if you prefer.
    with cf.ThreadPoolExecutor(max_workers=MAX_WORKERS_FOR_LEAFS) as pool:
        futures = {pool.submit(process_leaf_dir, leaf): leaf for leaf in leafs}
        for fut in cf.as_completed(futures):
            # bubble up exceptions to fail fast with clear context
            leaf = futures[fut]
            try:
                fut.result()
            except Exception as e:
                log_error(f"Error processing leaf {leaf}: {e}")
