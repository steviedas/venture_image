...
path_to_process = r"C:\Users\stevi\Desktop\Unprocessed Photos"
    ...
    for root, _, filenames in os.walk(path_to_process):
        for filename in filenames:
            ...
            if ext not in image_extensions:
                # Unsupported file -> move to unprocessable
                safe_move(full_path, os.path.join(unprocessable_path, filename))
                continue
...
def process_images():
    ...
    hash_map = {}  # hash -> (main_index, dupes_count, main_date, main_size, main_path)
    ...
    for full_path, ext, file_hash, date_taken, file_size in files:
        if file_hash not in hash_map:
            ...
            if file_size > main_size:
                # Keep new one as main, demote old one to dupe
                dupes_count += 1
