# vi_app/modules/dedup/service.py
...
def _select(strategy: DedupStrategy):
    """Return a strategy instance based on the enum."""
...
def _bump_until_free(dst: Path) -> Path:
    ...
    if base and tail.endswith(")"):
        ...
        try:
            ...
            # not a standard pattern; append fresh suffix
            base = stem
            n = 1
    else:
        base = stem
...
def apply(
    ...
    """
    clusters = plan(req, reporter=reporter)
    if req.dry_run:
        return clusters
    ...
    def _moves() -> Iterable[tuple[Path, Path]]:
        for cluster in clusters:
            ...
            for dup in cluster.duplicates:
                ...
                # - explicit req.move_duplicates_to
                # - otherwise a sibling 'duplicate' folder next to the duplicate
